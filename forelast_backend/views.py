...
class WeatherAnalyticsAPI(View):
    def get(self, request, city):
        try:
            ...
            table_name = self._get_table_name(city)
            
            # Get historical data (14 days)
            historical = self._fetch_historical_data(supabase, table_name)
            
            # Get forecast data (7 days)
            forecast = self._fetch_forecast_data(supabase, f"{table_name}")
...
# views.py
    def _get_table_name(self, city):
        """Normalize city name for table lookup with all special cases"""
        ...
        special_cases = {
            "las_piñas": "las_pinas",
            "marikina": "markina",
            "parañaque": "paranaque",  # Fixed typo from previous version
            "caloocan": "caloocan",    # Common alternative spelling
            "quezon_city": "quezon",
            "manila": "manila"
        }
        # Return either the special case or original
        return f"{special_cases.get(city, city)}_city_forecast"
